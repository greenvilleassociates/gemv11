...
<form id="form1" runat="server">
	<div class="auto-style1">
		...
		<br><br>COCKY ENTERPRISES <br>FIREWALL CONTEXT MAINTENANCE<br><br>
		<div class="auto-style1">
		</div>
	</div>
	<div align="center">
	<asp:SqlDataSource id="SqlDataSource1" runat="server" ConflictDetection="CompareAllValues" ConnectionString="Dsn=GEMPROD" DeleteCommand="DELETE FROM [gfirewallcontexts] WHERE [id] = ? AND (([firewallid] = ?) OR ([firewallid] IS NULL AND ? IS NULL)) AND (([contextid] = ?) OR ([contextid] IS NULL AND ? IS NULL)) AND (([description] = ?) OR ([description] IS NULL AND ? IS NULL)) AND (([ipaddress] = ?) OR ([ipaddress] IS NULL AND ? IS NULL)) AND (([telemetryipaddress] = ?) OR ([telemetryipaddress] IS NULL AND ? IS NULL)) AND (([vpnid1] = ?) OR ([vpnid1] IS NULL AND ? IS NULL)) AND (([vpnid2] = ?) OR ([vpnid2] IS NULL AND ? IS NULL)) AND (([inventoryid] = ?) OR ([inventoryid] IS NULL AND ? IS NULL))" InsertCommand="INSERT INTO [gfirewallcontexts] ([firewallid], [contextid], [description], [ipaddress], [telemetryipaddress], [vpnid1], [vpnid2], [inventoryid]) VALUES (?, ?, ?, ?, ?, ?, ?, ?)" OldValuesParameterFormatString="original_{0}" ProviderName="System.Data.Odbc" SelectCommand="SELECT * FROM [gfirewallcontexts]" UpdateCommand="UPDATE [gfirewallcontexts] SET [firewallid] = ?, [contextid] = ?, [description] = ?, [ipaddress] = ?, [telemetryipaddress] = ?, [vpnid1] = ?, [vpnid2] = ?, [inventoryid] = ? WHERE [id] = ? AND (([firewallid] = ?) OR ([firewallid] IS NULL AND ? IS NULL)) AND (([contextid] = ?) OR ([contextid] IS NULL AND ? IS NULL)) AND (([description] = ?) OR ([description] IS NULL AND ? IS NULL)) AND (([ipaddress] = ?) OR ([ipaddress] IS NULL AND ? IS NULL)) AND (([telemetryipaddress] = ?) OR ([telemetryipaddress] IS NULL AND ? IS NULL)) AND (([vpnid1] = ?) OR ([vpnid1] IS NULL AND ? IS NULL)) AND (([vpnid2] = ?) OR ([vpnid2] IS NULL AND ? IS NULL)) AND (([inventoryid] = ?) OR ([inventoryid] IS NULL AND ? IS NULL))">
		<DeleteParameters>
			<asp:Parameter Name="original_id" Type="Int32" />
			<asp:Parameter Name="original_firewallid" Type="String" />
			<asp:Parameter Name="original_firewallid" Type="String" />
			<asp:Parameter Name="original_contextid" Type="String" />
			<asp:Parameter Name="original_contextid" Type="String" />
		...
		<InsertParameters>
			<asp:Parameter Name="firewallid" Type="String" />
			<asp:Parameter Name="contextid" Type="String" />
	...
	<asp:DetailsView id="DetailsView1" runat="server" AutoGenerateRows="False" DataKeyNames="id" DataSourceID="SqlDataSource1" Height="50px" Width="622px" AllowPaging="True">
		<Fields>
			...
			<asp:BoundField DataField="firewallid" HeaderText="firewallid" SortExpression="firewallid">
			</asp:BoundField>
			<asp:BoundField DataField="contextid" HeaderText="contextid" SortExpression="contextid">
			</asp:BoundField>
...
<a href="managefirewallcontexts.aspx">Return to Firewall Context Manager</a>
</div>
</body>
</html>