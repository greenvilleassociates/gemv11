...
function pureblog_custom_colors_css() {
	$hue = absint( get_theme_mod( 'colorscheme_hue', 3 ) );

	/**
	 * Filter PureBlog default saturation level.
	 * @since PureBlog 1.0.0
	 * @param int $saturation Color saturation level.
	 */
	$saturation = absint( apply_filters( 'pureblog_custom_colors_saturation', 50 ) );
	$reduced_saturation = ( .8 * $saturation ) . '%';
	$reduced_saturation2 = ( .4 * $saturation ) . '%';
	$reduced_saturation3 = ( 0 * $saturation ) . '%';
	$saturation = $saturation . '%';
	$css = '
/**
 * PureBlog: Color Patterns*/
.colors-custom #banner {
	background-color: hsl( ' . $hue . ', ' . $reduced_saturation3 . ', 95% ); /* base: #f5f5f5; */
}
.colors-custom #bottom-sidebars {
	background-color: hsl( ' . $hue . ', ' . $reduced_saturation2 . ', 93% ); /* base: #ecece2; */
}
.colors-custom hr {
	background: hsl( ' . $hue . ', ' . $saturation3 . ', 73% ); /* base: #3c3c3c; */
}
.colors-custom .featured-post { 
	color: hsl( ' . $hue . ', ' . $saturation . ', 42% ); /* base: #c38c89; */
}
.colors-custom #right-sidebar .widget li a,
.colors-custom #left-sidebar .widget li a {
	border-bottom-color: hsl( ' . $hue . ', ' . $reduced_saturation3 . ', 70% ); /* base: #b3b3b3; */
	color: hsl( ' . $hue . ', ' . $reduced_saturation3 . ', 45% ); /* base: #b3b3b3; */
}
.colors-custom table th {
	background-color: hsl( ' . $hue . ', ' . $saturation . ', 93% ); /* base: #ecece2; */	
}
...
.colors-custom .dropdown-toggle,
.colors-custom .menu-toggle {
	color: hsl( ' . $hue . ', ' . $saturation . ', 100% ); /* base: #222; */
}
...
.colors-custom .footer-copyright a {
	color: hsl( ' . $hue . ', ' . $reduced_saturation3 . ', 40% ); /* base: #71757b; */
}
.colors-custom abbr,
.colors-custom acronym {
	border-bottom-color: hsl( ' . $hue . ', ' . $reduced_saturation3 . ', 40% ); 
}

.colors-custom .entry-meta,
.colors-custom .entry-meta a {
	color: hsl( ' . $hue . ', ' . $reduced_saturation3 . ', % ); /* base: #999; */
}
...
.colors-custom .navigation-top .current_page_item > a,
.colors-custom .main-navigation a:hover {
	color: hsl( ' . $hue . ', ' . $saturation . ', 63% ); /* base: #fff; */
}
...
.colors-custom .site-content .wp-playlist-light .wp-playlist-item:focus {
	color: hsl( ' . esc_attr( $hue ) . ', ' . esc_attr( $saturation ) . ', 46% );
}
...
.colors-custom .wp_widget_tag_cloud a:focus {
	border-color: hsl( ' . $hue . ', ' . $reduced_saturation3 . ', 80% ); /* base: #bbb; */
}

.colors-custom thead th {
	border-bottom-color: hsl( ' . $hue . ', ' . $reduced_saturation3 . ', 73% ); /* base: #bbb; */
}
...
.colors-custom .wp_widget_tag_cloud a {
	border-color: hsl( ' . $hue . ', ' . $reduced_saturation3 . ', 90% ); /* base: #ddd; */
}

.colors-custom.pureblog-front-page article:not(.has-post-thumbnail):not(:first-child),
.colors-custom .widget ul li {
	border-top-color: hsl( ' . $hue . ', ' . $reduced_saturation3 . ', 90% ); /* base: #ddd; */
}

.colors-custom .widget ul li {
	border-bottom-color: hsl( ' . $hue . ', ' . $reduced_saturation3 . ', 90% ); /* base: #ddd; */
}
...
.colors-custom tr {
	border-bottom-color: hsl( ' . $hue . ', ' . $reduced_saturation3 . ', 93% ); /* base: #eee; */
}
...
.colors-custom .single-featured-image-header {
	background-color: hsl( ' . $hue . ', ' . $reduced_saturation3 . ', 10% ); /* base: #1b1b1b; */
}
...
@media screen and (min-width: 48em) {
	...
	.colors-custom .main-navigation ul ul {
		border-color: hsl( ' . $hue . ', ' . $saturation . ', 40% ); /* base: #bbb; */
	...
	.colors-custom .main-navigation ul li.page_item_has_children:after {
		border-bottom-color: hsl( ' . $hue . ', ' . $reduced_saturation3 . ', 100% ); /* base: #fff; */
	}
	.colors-custom .main-navigation li li a {
		color: hsl( ' . $hue . ', ' . $saturation . ', 15% ); /* base: #767676; */
	}
	...
	/**
	 * Filters PureBlog custom colors CSS.
	 *
	 * @since PureBlog 1.0.0
	 *
	 ...
	 */
	return apply_filters( 'pureblog_custom_colors_css', $css, $hue, $saturation );
}
